...
class KeyChainManager: NSObject {
    // TODO: 创建查询条件
    class func createQuaryMutableDictionary(account:String, withService service:String)->NSMutableDictionary{
        // 创建一个条件字典
        ...
        // 设置存储数据的标记
//        keychainQuaryMutableDictionary.setValue("888", forKey: kSecAttrAccessGroup as String)
        keychainQuaryMutableDictionary.setValue(service, forKey: kSecAttrService as String)
        keychainQuaryMutableDictionary.setValue(account, forKey: kSecAttrAccount as String)
        // 设置数据访问属性
    ...
    // TODO: 存储数据
    class func keyChainSaveData(data:Any ,withAccount account:String, withService service:String)->Bool {
        // 获取存储数据的条件
        let keyChainSaveMutableDictionary = self.createQuaryMutableDictionary(account: account, withService: service)
        // 删除旧的存储数据
    ...
    // TODO: 更新数据
    class func keyChainUpdata(data:Any ,withAccount account:String,withService service:String)->Bool {
        // 获取更新的条件
        let keyChainUpdataMutableDictionary = self.createQuaryMutableDictionary(account: account, withService: service)
        // 创建数据存储字典
    ...
    // TODO: 获取数据
    class func keyChainReadData(account:String, withService service:String)-> Any {
        var idObject:Any?
        // 获取查询条件
        let keyChainReadmutableDictionary = self.createQuaryMutableDictionary(account: account, withService: service )
        // 提供查询数据的两个必要参数
    ...
    // TODO: 删除数据
    class func keyChianDelete(account:String, withService service:String)->Void{
        // 获取删除的条件
        let keyChainDeleteMutableDictionary = self.createQuaryMutableDictionary(account: account, withService: service)
        // 删除数据
